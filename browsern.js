...
async function CustomPageTurnstile(page) {
    ...
    if (Turnstile) {
      try {
         ...
         console.log('Turnstile checked:', result);
         await sleep(3);
      } catch (error) {
...
async function JsChallenge(page) {
    ...
    try {
       if (content.includes("https://waf.chaitin.com/challenge/v2/challenge.css")) {
         ...
         if (captcha) {
           console.log('[INFO] Detected protection ~ SafeLine WAF Click Challenge');
       ...
       if (content.includes("https://challenges.cloudflare.com/turnstile/v0/api.js")) {
         console.log('[INFO] Detected protection ~ Turnstile');
         await page.screenshot({ path: 'sebelum.png', fullPage: true });
         await sleep(3);
         const result = await checkTurnstile({ page });
         console.log('Turnstile checked:', result);
       ...
       if (content.includes("/vddosw3data.js")) {
...
async function detectChallenge(browser, page, client) {
    ...
    if (content.includes("challenge-platform")) {
        try {
            ...
            const selector = 'body > div.main-wrapper > div > div > div > div';
